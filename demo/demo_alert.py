...
class RailwayAlert:
    ...
    def dec_alert(self, test_path):
        ...
        :return: 态势评估结果
        """
        img, ori_w, ori_h = self.load_img(test_path)

        results = detect(self.dec_model, img, self.decoder,
                         self.input_w, self.input_h, ori_w, ori_h)

        # 存在潜在危险的类别，可活动的地面类别
        alter_cls = ['small-vehicle', 'large-vehicle', 'helicopter', 'plane', 'ship']
        protect_cls = ['train', 'rail']

        pass

    def seg_alert(self, base_path, test_path):
        """
        基于语义分割的态势评估
