...
class TestModule(object):
    ...
    def test(self, args, down_ratio):
        save_path = os.path.join('runs', args.dataset)
        ...
        for cnt, data_dict in enumerate(data_loader):
            image = data_dict['image'][0].cuda()
            img_id = data_dict['img_id'][0]  # img name
            print('processing {}/{} image, cur: {}'.format(cnt, len(data_loader), img_id))
            ...
            ori_image = dsets.load_image(img_id)
            ori_h, ori_w, _ = ori_image.shape
            pts0, scores0 = func_utils.decode_prediction(predictions, dsets.category,
            ...
            for cat in dsets.category:
                ...
                pts_cat = np.asarray(pts_cat, np.float32)  # n,4,2
                scores_cat = np.asarray(scores_cat, np.float32)  # n,
                if pts_cat.shape[0]:
            ...
            #     gt_anno = dsets.load_annotation(img_id)
