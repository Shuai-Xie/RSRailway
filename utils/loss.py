...
class OffSmoothL1Loss(nn.Module):
    ...
    def forward(self, output, mask, ind, target):
        """
        :param output: torch.Size([1, 2, 152, 152]), 1/4 feature map, 2 channels
        :param mask: torch.Size([1, 500])
        :param ind: torch.Size([1, 500])
        :param target: torch.Size([1, 500, 2])
        :return:
        """
...
class LossAll(torch.nn.Module):
    ...
    def forward(self, pr_decs, gt_batch):
        """
        :param pr_decs:  {'hm', 'reg', 'wh', 'cls_theta'}
        :param gt_batch: {
                            'input': image,
                            'hm': hm,
                            'reg_mask': reg_mask,
