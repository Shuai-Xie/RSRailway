...
class DecDecoder(object):
    ...
    def _topk(self, scores):
        ...
        topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), self.K)  # b,c,K

        topk_inds = topk_inds % (height * width)
        topk_ys = (topk_inds // width).int().float()  # y
        topk_xs = (topk_inds % width).int().float()  # x

        topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), self.K)  # b,c*k
        topk_clses = (topk_ind // self.K).int()  # b,k
        topk_inds = self._gather_feat(topk_inds.view(batch, -1, 1), topk_ind).view(batch, self.K)
        topk_ys = self._gather_feat(topk_ys.view(batch, -1, 1), topk_ind).view(batch, self.K)  # b,k
        topk_xs = self._gather_feat(topk_xs.view(batch, -1, 1), topk_ind).view(batch, self.K)  # b,k 每张图 k 个结果

        return topk_score, topk_inds, topk_clses, topk_ys, topk_xs

    def _nms(self, heat, kernel=3):  # point nms
    ...
    def ctdet_decode(self, pr_decs):
        ...
        scores, inds, clses, ys, xs = self._topk(heat)  # b,k
        ...
        mask = (cls_theta > 0.8).float().view(batch, self.K, 1)  # mask=1, RBB
        # 将 RBB 和 HBB 统一计算方式
        ...
        #
        detections = torch.cat([xs,  # cen_x, b,k,1
                                ys,  # cen_y
                                tt_x,  # 4 中点 坐标
                                tt_y,
                                rr_x,
